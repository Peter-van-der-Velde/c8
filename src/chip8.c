...
void read_rom(char* path)
{
	...
	size = (size_t)ftell(ptr); /*calc the size needed*/
...
void emulate_cycle(void)
{
	...
	switch (opcode & 0xF000) {
		case 0x0000:
			op_0000(opcode);
			break; // 0x00E0: Clears the screen [OR] 0x00EE: Returns from subroutine
		case 0x1000:
			op_1000(opcode);
		...
		case 0x3000:
			op_3000(opcode);
			break; // Skip next instruction if Vx = kk.
		case 0x4000:
			op_4000(opcode);
			break; //
		case 0x5000:
		...
		case 0xA000:
			op_A000(opcode);
			break; // ANNN: Sets I to the address NNN
		case 0xB000:
			op_B000(opcode);
			break; //
		case 0xC000:
